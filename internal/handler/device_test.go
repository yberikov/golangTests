...
import (
	...
	"homework/internal/handler/validate"
...
func TestHandler_handleGetDevice(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedError: storage.ErrNoSuchDevice,
			respErr:       storage.ErrNoSuchDevice,
		},
		{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.respErr == nil {
				serviceMock.On("GetDevice", tt.serialNum).
					Return(models.Device{SerialNum: tt.serialNum, Model: "HP", IP: "111.111.111.111"}, nil).Maybe()
			} else {
				serviceMock.On("GetDevice", tt.serialNum).
					Return(models.Device{}, tt.respErr).Maybe()
			}
...
func TestHandler_handleCreateDevice(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler := h.InitRoutes()
			if tt.respErr == nil {
				serviceMock.On("CreateDevice", models.Device{SerialNum: tt.serialNum, Model: tt.model, IP: tt.ip}).
					Return(nil).Maybe()
			} else {
				serviceMock.On("CreateDevice", models.Device{SerialNum: tt.serialNum, Model: tt.model, IP: tt.ip}).
					Return(tt.respErr).Maybe()
...
func TestHandler_handleUpdateDevice(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler := h.InitRoutes()
			if tt.respErr == nil {
				serviceMock.On("UpdateDevice", models.Device{SerialNum: tt.serialNum, Model: tt.model, IP: tt.ip}).
					Return(nil).Maybe()
			} else {
				serviceMock.On("UpdateDevice", models.Device{SerialNum: tt.serialNum, Model: tt.model, IP: tt.ip}).
					Return(tt.respErr).Maybe()
...
func BenchmarkCreateDeviceHandle(b *testing.B) {
	b.Run("Endpoint: /createDevice", func(b *testing.B) {
		...
		h := &Handler{
			service: service.NewService(storage.NewDeviceStorage()),
		}
		handler := h.InitRoutes()
